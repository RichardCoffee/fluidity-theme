...
class TCC_Query_TermCount {
	...
	public function __construct( $args = array() ) {
		...
		if ( ! empty( $this->counts ) ) {
			add_filter( 'wp_nav_menu_objects', array( $this, 'add_navmenu_catagory_count' ) ); }
	}
	...
	public function add_navmenu_catagory_count( $items ) {
		foreach( $items as $item ) {
			...
			if ( ( intval( $item->menu_item_parent, 10 ) > 0 ) && array_key_exists( $item->object_id, $this->counts ) ) {
				...
				$item->title = sprintf( '%s <span class="%s">%s</span>', $item->title, esc_attr($css), number_format_i18n( $this->counts[ $item->object_id ] ) );
			}
		}
		return $items;
	}
